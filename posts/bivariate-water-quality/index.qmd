...
Traditional maps use just a single colour scale to display a single variable (@fig-uni-maps). Displaying them side by side isn't a bad option, but it's challenging but it’s challenging to compare locations with high stock density and low *E. coli* (and vice versa). Bivariate maps address this challenge.
...
dat |>
  ...
  labs(
    ...
    colour = "Modelled median <br>concentration of *E. coli* <br>(cfu/100mL)"
  ) -> ecoli_map
...
-   Manawatū-Whanganui, Hawke's Bay, Wairarapa, Marlborough, Canterbury high country, and Otago have high sheep density but relatively low levels of nitrogen.
...
-   Some areas of Northland, Waikato, Taranaki, Manawatū-Whanganui, and Southland have both high levels of dairy density and *E. coli*.
...
This analysis highlights the complex relationship between livestock density and water quality in New Zealand (@fig-overview). Using bivariate colour scale maps, we can visually explore these interactions and identify regions where specific patterns emerge.

![Bivariate comparison of upstream livestock density to river water quality.](labelled-total-ecoli.png){#fig-overview}
